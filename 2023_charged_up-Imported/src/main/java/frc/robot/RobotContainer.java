    ...
    switch (m_autoSelected){
      ...
      case(kPreloadBackup):
        ...
        return new SequentialCommandGroup(
          ...
          new WaitCommand(.5),
          //new setIntakeIn(intake),
          new taxi(drivetrain).withTimeout(2),
          new rotate(drivetrain),
          new setIntakeIn(intake),
          new InstantCommand(()->{intake.runIntakeIn(.4);}),
          new pickup(drivetrain).withTimeout(1.825),
          new InstantCommand(()->{intake.toggle();}),
          new WaitCommand(.125),
          new InstantCommand(()->{intake.runIntakeIn(0.05);}),
          new setIntakeOut(intake),
          new rotateback(drivetrain),
          new taxiback(drivetrain).withTimeout(.5),
          new setIntakeHalf(intake).withTimeout(.25),
          new InstantCommand(()->{
            System.out.print("shooting cone");
            intake.runIntakeOut(.8);
          })
